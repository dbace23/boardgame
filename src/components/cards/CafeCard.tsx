...
const CafeCard: React.FC<CafeCardProps> = ({ 
  ...
  return (
    <BaseCard id={cafe.id} linkTo={`/cafe/${cafe.id}`} className={className}>
      <ImageHeader image={cafe.image} alt={cafe.name} aspectRatio="wide" />
      ...
      <div className="p-4">
        <div className="flex justify-between items-start mb-2">
          ...
          <button 
            ...
            className={`p-1.5 rounded-full transition-colors ${
              cafe.isFollowing ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
        ...
        <div className="flex items-center mb-2 text-sm text-gray-600">
          ...
          <span>{cafe.openingHours.weekdays}</span>
        </div>
